...
keep <- c("Sedentary; 9 days of a high-fat diet alone (Sed-HFD)")
phdf <- phdf[phdf$`group:ch1` %in% keep, ]
counts <- counts[rownames(counts) %in% rownames(phdf), ]
fsn <- as.factor(phdf$source_name_ch1)

x <- c("source_name_ch1Sedentary_Post_muscle-source_name_ch1Sedentary_Pre_muscle")
results <- limma.rnaseq(~ 0 + source_name_ch1, phdf, counts, contrasts = x, random_effect="subject id:ch1")